...
namespace GGUI
{
	const unsigned int g_DXVertexFVF = (D3DFVF_XYZRHW|D3DFVF_DIFFUSE|D3DFVF_TEX1);
	...
	//-----------------------------------------------------------------------------
	bool GGUIDXRenderManager::CreateDXVertexBuffer(int nCapacity)
	{
		//尝试删除已有的DX顶点缓冲区。
		SAFE_D3D_RELEASE(m_pDXVertexBuffer);
		//创建DX顶点缓冲区。
	...
	//-----------------------------------------------------------------------------
	bool GGUIDXRenderManager::CreateDXIndexBuffer(int nCapacity)
	{
		//尝试删除已有的DX索引缓冲区。
		SAFE_D3D_RELEASE(m_pDXIndexBuffer);
		//创建DX索引缓冲区。
		...
		int nRectCount = m_nDXIndexCapacity / 2;
		int nRectIndex = 0;
		while (nRectIndex < nRectCount)
		{
			int nVertexIndex = nRectIndex * 4;

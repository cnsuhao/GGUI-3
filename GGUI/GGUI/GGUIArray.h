...
namespace GGUI
{
	...
	public:
		...
		GGUIArray(const Temp& invalidValue, int nInitCapacity);
		~GGUIArray();
		...
		//获取一个元素。
		Temp& GetElement(int nIndex);
		const Temp& GetElement(int nIndex) const;
	...
	//-----------------------------------------------------------------------------
	template <class Temp>
	inline GGUIArray<Temp>::GGUIArray(const Temp& invalidValue, int nInitCapacity)
	...
	inline void GGUIArray<Temp>::AddElement(const Temp& theElement)
	{
		...
		if (m_nWriteIndex >= m_nCapacity)
		{
			...
			unsigned int sizeOfOldArray = sizeof(Temp) * m_nCapacity;
			m_nCapacity *= 2;
			unsigned int sizeOfNewArray = sizeof(Temp) * m_nCapacity;
	...
	//-----------------------------------------------------------------------------
	template <class Temp>
	inline void GGUIArray<Temp>::RemoveElement(int nIndex)
	{
	...
	//-----------------------------------------------------------------------------
	template <class Temp>
	inline Temp& GGUIArray<Temp>::GetElement(int nIndex)
	{
		if (nIndex>=0 && nIndex<m_nWriteIndex)
		{
	...
	//-----------------------------------------------------------------------------
	template <class Temp>
	const Temp& GGUIArray<Temp>::GetElement(int nIndex) const
	{
		if (nIndex>=0 && nIndex<m_nWriteIndex)
		{
			return m_Array[nIndex];
	...
	//-----------------------------------------------------------------------------
	template <class Temp>
	inline int GGUIArray<Temp>::GetWriteIndex() const
	{
	...
	//-----------------------------------------------------------------------------
	template <class Temp>
	inline int GGUIArray<Temp>::GetCapacity() const
	{
		return m_nCapacity;
	}
} //namespace GGUI
